...
<script>
    ...
    function updateRegistroGaleria(Id,Title,Description){
        $('#image-loader').show();
        $("#pictures_list").load('/<?php echo $app_name ?>.php/gproyectos/updateRegistroGaleria',
            ...
            $("#gallery" ).sortable({
                    ...
                    $('#image-loader').show();
                     var order = $('#gallery').sortable('serialize');
                         //alert(order);
                     $("#pictures_list").load('/<?php echo $app_name ?>.php/proyectos/ajaxRegistroOrder?id=<?php echo $list_registros[0]->getPublicacionId();?>&'+order,
                     {},
                     function(){
                          $('#image-loader').hide();
                     });
                    ...
                    <?php if($list_registros->count() > 0): ?>
                        ...
                        <?php foreach( $list_registros as $i=>$registro ): ?>
                            ...
                            <li id="registro-<?php echo $registro->getId()?>" class="ui-widget-content ui-corner-tr picture">
                                    <h5 class="ui-widget-header titulo-galeria"><?php echo $registro->getTituloCortoBackend(10); ?></h5>
                                    <div id="image_<?php echo $registro->getId(); ?>">
                                        <!--a href="<?php echo $registro->getArchivoValid()?>" title="<?php echo $registro->getContenido()?>" rel="prettyPhoto[pp_gal]" style="padding: 0px; margin:0px;"-->
                                            <img class="basic" src="<?php echo $registro->getThumbnailValid(); ?>"  title="<?php echo $registro->getTitulo(); ?>" descripcion="<?php echo $registro->getContenido(); ?>" url="<?php echo url_for('proyectos_galeria_edit',$registro,true)?>" />
                                        <!--/a-->
                                    </div>
                                    <a href="#" onclick="titularizar('<?php echo $registro->getId() ?>')" title="Editar registro" class="ui-icon ui-icon-pencil">Editar registro</a>
                                    <a href="#" onclick="eliminar('<?php echo $registro->getId() ?>','<?php echo url_for('proyectos_galeria_ajax_registro_delete', $registro) ?>')" title="Eliminar registro" class="ui-icon ui-icon-trash">Eliminar registro</a>
                            </li>
	...
	<div>
		...
		<div class="content">
			...
			<script type="text/javascript">
			  tinyMCE.init({
				mode:                              "exact",
				elements:                          "proyectos_galeria_contenido",
				theme:                             "advanced",
