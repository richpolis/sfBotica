...
class sfRichSys
{
    ...
    static public function crearThumbnailCuadrado($path,$imagen,$New=false,$eliminarFuente=false){
        $fileImagen = $path.$imagen;
        if($New){
            chmod($fileImagen, 0666);
        }    
    ...
    static public function crearImagenClientes($path,$imagen,$New=false,$eliminarFuente=false){
        $fileImagen = $path.$imagen;
        if($New){
            chmod($fileImagen, 0666);
        }    
        ...
        $img = new sfImage($fileImagen,self::getTipoMime($imagen));
        $img->thumbnail(200,100,'center');
        
        $img->saveAs($fileImagen);
