...
class ProyectosTable extends Doctrine_Table
{
    ...
    public function getCriteriaUltimasPublicaciones(){
        ...
        $q->leftJoin($rootAlias . '.CategoriasProyectos c');
        return $q;
    }
    ...
    public function getPublicacionConGaleria($publicacion){
        ...
        $rootAlias = $q->getRootAlias();
        $q->leftJoin($rootAlias . '.ProyectosGaleriaJerryML g');
        return $q->fetchOne();
    }
    
    public function getProyectosConGaleriaPorFecha($fecha_inicial,$fecha_final){
        $q=$this->getCriteriaOrdenada();
        $q->addWhere('i.updated_at BETWEEN ? to ?',array($fecha_inicial,$fecha_final));
        $rootAlias = $q->getRootAlias();
        $q->leftJoin($rootAlias . '.ProyectosGaleria g');
    ...
    public static function getForLuceneQuery($query)
    {
        ...
        $q = Doctrine_Core::getTable('Proyectos')->addCriteriaPublicacionesActivas($q);
