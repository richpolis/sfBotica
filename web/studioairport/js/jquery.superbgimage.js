...
(function($) {
	...
	jQuery.fn.superbgimageFinished = function() {
			$.superbg_imagesLoaded++;
			var thumbli = $('.project-navigation').find('li[rel="'+$.superbg_imagesLoaded+'"]');
			thumbli.addClass('loaded'); //.show();
			if($.superbg_imagesLoaded == $('#' + $.superbg_saveId).children().length){ $('#loading').hide(); $.superbg_imagesLoaded = 1;}
	};
...
/*
			if($('#' + $.superbg_saveId).children('.preload').length == 1){
				$('#loading').hide();
				log('geen proload meer');
			}else{
				log('nog wel preload');
			}*/
	...
	jQuery.fn.superbgTransition = function(thisimg, dimensions) {
		...
		if ((typeof options.onClick === 'function') && (options.onClick !== null)) {
			$(thisimg).unbind('click').click(function(e) { options.onClick($.superbg_imgActual, e); });
		}
		if ((typeof options.onMouseDown === 'function') && (options.onMouseDown !== null)) {
			$(thisimg).unbind('mousedown').mousedown(function(e) { options.onMouseDown($.superbg_imgActual, e); });
		}
		if ((typeof options.onMouseUp === 'function') && (options.onMouseUp !== null)) {
			$(thisimg).unbind('mouseup').mouseup(function(e) { options.onMouseUp($.superbg_imgActual, e); });
		}
